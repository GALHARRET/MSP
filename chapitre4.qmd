...
![](figure/carte_ctrl1.png)
On se fixe un risque $\alpha$ de stoper la production alors que celle-ci est sous contrôle (***Fausses alertes***). On cherche donc un intervalle de confiance $1-\alpha$ de $\bar X$ 
La distribution des moyennes étant normale on a 
...
Pour l'estimation de $\mu_R$ on prend $\hat\mu_R=\bar R$, et pour l'estimation de $\sigma_R$ on prend $\hat \sigma_R=\frac{d_3}{d_2}\bar R$ où $d_3$ est l'écart type des étendues d'une loi normale centrée réduite.
On a alors
$$
\begin{cases}
