...
On considère $I$ produits, $J$ opérateurs et chaque mesure est repétée $K$ fois
Pour évaluer la répétabilité et la reproductibilité d'une mesure, on utilise un modèle d'ANOVA :
Soit $Y_{ijk}$ la mesure du produit $i$ par l'opérateur $j$ à la répétition $k$.  
On écrit 
$$
...
***Exemple :*** On cherche à détecter les sources de variabilité d’une analyse par qPCR
La réponse de la mesure dépend de la quantité d’ADN recherchée dans l’échantillon de départ.

- 4 espèces différentes de mycoplasmes sont étudiées.

- 2 opérateurs ont réalisé les mesures, sur 2 jours distincts, avec 4 répétitions à chaque fois.
Les données sont disponibles [ici](ex_qPCR.csv).
