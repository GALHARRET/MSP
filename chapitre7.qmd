...
1.   Etape 1 : Contribution individuelle. Calculer $$t^2_{j}=\frac{n(\bar{X}^{(t)}_{j}-\overline{ X}_j)^2}{s_j^2}$$ pour chaque variable $j$ où $s_j^2$ est la variance de la caractéristique $X^{(t)}_j$. Calculer pour $p=1$, Exclure les variables $j$ qui vérifient $t^2_j>LSC,$ où $LSC=\frac{(k+1)(k-1)}{k(k-1)}F_{(p,k-1)}(1-\alpha).$

```{=html}
<!-- -->
```
2.  Etape 2 : Contribution bivariée. On retire toutes variables identifiées à l'étape 1. On note $J$ les indices des variables restantes. Pour $j,j'\in J$, calculer $T_{jj'}^2$ correspondant aux colonnes $j,j'$ de la matrice $\textbf{X}_t$.
3.  Exclure les variables $j,j'$ qui vérifient $T^2_{jj'}>LSC,$ où $LSC=\frac{p(k+1)(k-1)}{k(k-p)}F_{(p,k-p)}(1-\alpha),$ pour $p=2.$
4.  On itère les étapes précédentes jusqu'au modèle qui contient toutes les variables restantes.
:::
...
=======
LSC=\frac{p(k+1)(k-1)}{k^2-kp}F_{p,m-p}(\alpha).$$
>>>>>>> b455c6501161b4d19580d5b2f7b50952699e2e12
