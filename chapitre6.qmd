...
On choisit $\lambda=0.1$.

```{r}
lambda=0.1
Z<-ewma_cart(M,mu=mu,sig=sig,lambda=lambda,L = 2.7)
plot_chart(Z$z,LIC=Z$LIC,LSC=Z$LSC,Type="Carte EWMA")
```

Si on considère que la déviation commence à partir du 11ième prélèvement alors la carte EWMA la détectera 8 prélèvements plus tard.
