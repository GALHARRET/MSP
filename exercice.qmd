# Exercices 

# Exercice 1 {.unnumbered}

- On considère le jeu de données [X](X.rda) qui correspond au suivi de 5 caractéristiques d'une production.

- On a prélevé 40 échantillons de 6 observations. Les 15 premiers prélèvements ont été effectués pendant 1 heure toutes les 10 min et vont servir à mettre en place les limites de contrôle. Les prélèvements suivants (16 à 40) ont été réalisés toutes les heures et 6 fois par jour.

```{r warning=FALSE, echo=TRUE}
library(multiSPC)
load("X.rda")
dim(X)
```

Il s'agit d'un cube de données (3 dimensions).


## Premier travail : cartes univariées sur la première caractéristique {.unnumbered}

Pour sélectionner la première caractéristique :

```{r}
data=X[,1,]
```

1. Construire les limites de contrôles de la carte de la moyenne et de l'écart type sur les 15 premiers prélèvements.

```{r eval=FALSE,echo=FALSE}
train=data[1:15,]
n=dim(data)[2]
M=apply(train,1,mean)
SD=apply(train,1,sd)
mu=mean(M)
set.seed("44")
c4=c4(n)
sig=mean(SD)/c4
LICm=mu-3*sig/sqrt(n)
LSCm=mu+3*sig/sqrt(n)
LICs=mean(SD)*(1-3*sqrt(1-c4^2)/c4)
LSCs=mean(SD)*(1+3*sqrt(1-c4^2)/c4)
plot_chart(M,LIC=LICm,LSC=LSCm,Type="Carte de la Moyenne")
plot_chart(SD,LIC=LICs,LSC=LSCs,Type="Carte de l'écart type")
```


2. Appliquer les limites précédentes sur les 25 prélèvements suivants. Que peut-on en déduire ?

```{r echo=F,eval=F}
test=data[16:40,]
Mt=apply(test,1,mean)
SDt=apply(test,1,sd)
plot_chart(Mt,LIC=LICm,LSC=LSCm)
plot_chart(SDt,LIC=LICs,LSC=LSCs)
```

3. Représenter la probabilité de détecter au premier prélèvement un décentrage $\mu_D$ variant de 25 à 29.

```{r echo=F, eval=F}
mu_D=seq(25,29,by=0.01)
proba=pnorm(LSCm,mu_D,sig,lower.tail = F)
df=data.frame(mu_D=mu_D,proba=proba)
library(ggplot2)
ggplot(df,aes(x=mu_D,y=proba))+
  geom_line()
```

4. Construction de la carte EWMA, on suivra les moyennes des prélèvements de la phase de test en considérant $\lambda=0.1,0.2,0.3,0.4$. Quel est l'impact de ces différentes valeurs sur la carte ? 


```{r echo=FALSE, eval=FALSE}
Z=ewma_cart(Mt,mu=mu,sig=sig,lambda=0.3,L=2.7/sqrt(n))
plot_chart(Z$z,LIC=Z$LIC,LSC=Z$LSC)
```



## Deuxième travail approche multivariée pour la construction des limites (15 premiers prélèvements) {.unnumbered}

On va construire deux cartes de contrôle multivariée sur les 15 premières observations afin de calculer les limites de contrôle qui seront utilisées pour les prélèvements en routine (25 dans notre cas).

-   Construire le graphe des corrélations des 5 caractéristiques sur la matrice $\tilde X$ de dimension $(k,p)$ dont les élements $$\tilde x_{ij}=\frac1{n}\sum_{m=1}^n x_{ijm}.$$

Fonction Moy_X() dans le package *multiSPC*.

```{r echo=F,eval=F}
Moy1=Moy_X(X1)
GGally::ggpairs(as.data.frame(Moy1))
```

-   Construire la carte de contrôle du $T^2$ de Hotelling. Que peut on en déduire ? Utiliser la fonction LSC_T2_Hotelling_kn() pour calculer les limites

```{r echo=F,eval=F}
res1=T2_Hotelling_kn(X1)
LSC1=LSC_T2_Hotelling_kn(X1)
plot_chart(res1,LSC=LSC1[1],Type="T2 Hotelling")
```

-   Construire la carte de contrôle du $T^2$ de Hotelling sur composantes principales à partir de la matrice $\tilde X$ définie précédemment. Justifier le nombre de composantes choisies.

```{r echo=F,eval=F}
resPC=Principal_Component_construction(Moy1,r=2,data2 = Moy_X(X2))
resPC$sing.values
resPC1=T2_Hotelling_k1(resPC$coord_PC1)
LSC2=LSC_T2_Hotelling_k1(resPC$coord_PC1)
plot_chart(resPC1,LSC=LSC2[1],Type="T2 Hotelling")
```

-   Que peut-on déduire de ces résultats ?

## Troisième travail approche multivariée pour la construction des cartes à partir des limites précédentes {.unnumbered}

Pour les 25 derniers prélèvements :

-   Construire la carte du $T^2$ de Hotelling en utilisant les limites Phase II obtenues à la question précédente.

```{r echo=F,eval=F}
res2=T2_Hotelling_kn(X2,
                    MoyT=Moy_T(X1),
                    S=covariance_X(X1))
plot_chart(res2,LSC=LSC2[2],Type="T2 Hotelling")
```

-   Construire la carte du $T^2$ de Hotelling sur Composantes principales en utilisant les limites Phase II obtenues à la question précédente.

```{r echo=F,eval=F}
resPC2=T2_Hotelling_k1(resPC$coord_PC2,
                       MoyT=Moy_T(resPC$coord_PC1),
                       S=covariance_X(resPC$coord_PC1))
plot_chart(resPC2,LSC=LSC2[2],Type="T2 Hotelling")
```

## Quatrième travail : Etude des points HC sur les cartes multivariées {.unnumbered}

-   Les points HC dans la carte du T2 seront étudiés en utilisant la fonction T2_Hotelling_kn_MYT() et en suivant la procédure décrite p20 du cours (CM2).

```{r echo=F,eval=F}
## HC 1 variable
k_hc=which(res2>LSC1[2])
MTY1=matrix(NA,nrow=length(k_hc),ncol=p)
rownames(MTY1)=k_hc
colnames(MTY1)=colnames(X1)
for(i in 1:length(k_hc)){
  for(j in 1:p){
  mty1=T2_Hotelling_kn_MYT(k_hc[i],k=15,select = j,MoyT = Moy_T(X1),data = X2)
  MTY1[i,j]=as.numeric(mty1$T2>=mty1$LSC)
}
}
MTY1
## Pour k_hc=5, caract2 éliminée

## HC deux variables
MTY2=matrix(NA,nrow=p,ncol=p)
rownames(MTY2)=colnames(MTY2)=colnames(X1)
for(i in 1:(p-1)){
  for(j in (i+1):p){
  mty2=T2_Hotelling_kn_MYT(k_hc[1],k=15,select = c(i,j),MoyT = Moy_T(X1),data = X2)
  MTY2[i,j]=as.numeric(mty2$T2>=mty2$LSC)
}
}
MTY2
# Pour k_hc=5, caract 1, 3 et 4 éliminées


for(i in 1:(p-1)){
  for(j in (i+1):p){
  mty2=T2_Hotelling_kn_MYT(k_hc[2],k=15,select = c(i,j),MoyT = Moy_T(X1),data = X2)
  MTY2[i,j]=as.numeric(mty2$T2>=mty2$LSC)
}
}
MTY2
# Pour k_hc=15 caract 3 et 4 éliminées


for(i in 1:(p-1)){
  for(j in (i+1):p){
  mty2=T2_Hotelling_kn_MYT(k_hc[3],k=15,select = c(i,j),MoyT = Moy_T(X1),data = X2)
  MTY2[i,j]=as.numeric(mty2$T2>=mty2$LSC)
}
}
MTY2
# Pour k_hc=24 caract 3,4,5 éliminées



## HC 3 Variables

T2_Hotelling_kn_MYT(k_hc[2],k=15,select = c(1,2,5),MoyT = Moy_T(X1),data = X2)

```

-   Les points HC de la carte $T^2$ sur composantes principales seront étudiées par des cartes univariées sur les composantes principales. On interprêtera les résultats à partir du cercle de corrélation. Pour cette carte $\sigma_I$ sera égal à :

```{r echo=F, eval=F}
k2=dim(X2)[1]
sig_I=sqrt(resPC$var_expl_PC*(k2-1))[1:2]
```

```{r echo=F,eval=F}
mu=apply(resPC$coord_PC1,2,mean)

plot_chart(resPC$coord_PC2[,1],LIC=mu[1]-3*sig[1]/sqrt(k),LSC=mu[1]+3*sig[1]/sqrt(k))
plot_chart(resPC$coord_PC2[,2],LIC=mu[2]-3*sig[2]/sqrt(k),LSC=mu[2]+3*sig[2]/sqrt(k))
```

## Conclusion {.unnumbered}

Conclure sur l'ensemble des résultats et interpréter les différences de résultats obtenus dans les différentes approches.


# Exercice 2 {.unnumbered}

Télécharger le fichier [dataEWMA](dataEWMA.csv). On suppose que pour les 12 premiers prélèvements la production est stable et centrée sur la qualité attendue.

1. Estimer les limites de contrôle de la moyenne et de l'écart type.

```{r echo=F, eval=F}
data=read.csv("dataEWMA.csv",row.names = 1)
training=data[1:12,]
M=apply(training,1,mean)
SD=apply(training,1,sd)
mu=mean(M)
n=4
library(multiSPC)
sigma=mean(SD)/c4(n)
LIC=mu-3*sigma/sqrt(n)
LSC=mu+3*sigma/sqrt(n)
plot_chart(M,LIC,LSC)
```



2. Appliquer les limites précédentes sur les prélèvements restants. Que remarque t'on ?

```{r echo=FALSE, eval=FALSE}
testing=data[13:40,]
Mt=apply(testing,1,mean)
plot_chart(Mt,LIC,LSC)
```

3. Construire la carte EWMA sur l'ensemble des prélèvements permettant de détecter le décentrage le plus rapidement possible sous la contrainte $0.1<\lambda<0.5$ et $2.5<L<3.0$. On reprendra les valeurs calculées dans la question pour $\mu,\sigma$.

```{r echo=FALSE, eval=FALSE}
M1=c(M,Mt)
lambda=seq(0.1,0.5,by=0.01)
L=seq(2.5,3,by=0.01)
ind<-matrix(NA,nrow=length(lambda),ncol=length(L))
for(i in 1:length(lambda)){
  for(j in 1:length(L)){
    Z=ewma_cart(M1,mu,sigma,lambda=lambda[i],L=L[j]/sqrt(n))
    ind[i,j]<-which(Z$z>Z$LSC)[1]
  }
}

M1[min(ind)]


```

4. En utilisant la carte de la moyenne, quelle serait la probabilité de détecter le premier point hors contrôle obtenu avec la carte EWMA ?

```{r echo=F, eval=F}
pnorm(LSC,mu,sigma,lower.tail = F)
```

